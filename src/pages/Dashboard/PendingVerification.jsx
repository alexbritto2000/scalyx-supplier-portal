...
const PendingVerification = () => {
    ...
    const handlePageChange = (newPage) => {
        refreshPendingVerification(newPage, pendingVerificationLimit);
    };

    const handleLimitChange = (newLimit) => {
        refreshPendingVerification(1, newLimit);
    };
        ...
        <div className="mt-8">
            <div className='flex justify-between'>
                <div className='font-bold text-[1.5rem]'>
                    Pending Verification ({totalPendingVerification})
                </div>
            ...
            <div className="overflow-auto rounded-xl border border-[#F0F0F0] bg-white p-4 shadow-[inset_0_0_4px_0_#334A5F1F] mt-[2rem]">
                {pendingVerificationLoading ? (
                    ...
                    <div className="py-8 text-center text-red-500">{pendingVerificationError}</div>
                ) : pendingVerification.length === 0 ? (
                    <div className="py-12 text-center text-gray-500">
                        <div className="text-lg font-medium mb-2">No Pending Verifications</div>
                        <div className="text-sm">There are no pending verifications to display at the moment.</div>
                    </div>
                ) : (
                    <>
                        <table className="min-w-full table-auto text-sm text-left text-gray-700">
                            ...
                            <tbody>
                                {pendingVerification.map((order, index) => {
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3 flex items-center gap-2">
                                                <motion.button
                                                    ...
                                                    onClick={() => updateOrderStatus(order.po_id, 'pending_shipment')}
                                                >
                                                    <div className="flex items-center gap-2">
                                                        <motion.img
                        ...
                        {/* Pagination */}
                        {totalPendingVerification > pendingVerificationLimit && (
                            <div className="flex justify-between items-center mt-4 pt-4 border-t border-[#F0F0F0]">
                                <div className="text-sm text-gray-600">
                                    Showing {((pendingVerificationPage - 1) * pendingVerificationLimit) + 1} to {Math.min(pendingVerificationPage * pendingVerificationLimit, totalPendingVerification)} of {totalPendingVerification} results
                                </div>
                                <div className="flex items-center gap-2">
                                    <select
                                        value={pendingVerificationLimit}
                                    ...
                                    <div className="flex gap-1">
                                        <button
                                            onClick={() => handlePageChange(pendingVerificationPage - 1)}
                                            disabled={pendingVerificationPage <= 1}
                                        ...
                                        <button
                                            onClick={() => handlePageChange(pendingVerificationPage + 1)}
                                            disabled={pendingVerificationPage * pendingVerificationLimit >= totalPendingVerification}
                                            className="px-3 py-1 text-sm border border-gray-300 rounded disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50"
                                        >
                                            Next
                                        </button>
